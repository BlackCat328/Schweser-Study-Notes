...
app.use(async (ctx) => { // context 上下文，request ---> 请求信息 响应信息
    ...
    if (ctx.request.url === "/App.vue") {
        const vueContent = await fs.promises.readFile(path.resolve(__dirname, "./main.js"));
        // 如果是 vue 文件，会做一个字符串替换：vueContent.toString().find("<template>") 如果匹配到了就直接全部进行字符串替换
        // AST语法解析 ==> Vue.createElement() --> 构建原生的dom
